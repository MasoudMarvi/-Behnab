...
class PackageList extends Component {
    ...
    componentDidMount() {
        ...
        this.getPackages(CatId);
    }
    async getPackages(id) {
        try {
            this.setState({loading: true});
            let token = await AsyncStorage.getItem('token');
            let url = 'http://upkon.ir/api/ApiPackages/Package';
            ...
            fetch(url, {
                ...
                .then((responseJson) => {
                    ...
                    if (responseJson.Ok === true) {
                        this.setState({
                            packages: responseJson.PackageResult
                        });
                    } else {
                        Toast.show({
                ...
                <Drawer ref={(ref) => {
                    ...
                    <Container>
                        <Header style={{backgroundColor: '#ffffff'}}>
                            ...
                            <Right>
                                <Title style={styles2.headerTitleText}>لیست پکیج ها</Title>
                        ...
                        <List style={{marginVertical: 20}}>
                                {this.state.packages.map((item, key) => (
                                    <ListItem key={key}
                                              onPress={() => navigate('BookPackage', {PackageId: item.Id})}
