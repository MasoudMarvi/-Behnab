                            ...
                            <Tabs renderTabBar={() => <ScrollableTab/>}
                                ...
                                {this.state.categories.map((item, key) => (
                                    <Tab heading={item.Name} key={key}
                                        ...
                                        <List>
                                            {this.state.currentTabItems.map((pack, i) => (
                                                <ListItem key={i}>
                                                    <Body>
                                                    <Text style={[styles.listText, styles.bold]}>{pack.Name}</Text>
                                                    <Button small
                                                            style={{paddingRight: 2, borderRadius: 40,}}
                                                            onPress={() => navigate('PackageList', {CatId: pack.Id})}>
                                                        <Icon type="FontAwesome" name="chevron-left"
                                                              style={{fontSize: 12}}/>
                                                    </Button>
                                                    </Body>
                                                    <Thumbnail circle size={120}
                                                               source={pack.ImageUrl !== null ? {uri: String('http://upkon.ir/' + pack.ImageUrl)} : require('../img/1.png')}/>
                                                </ListItem>
                                            ))}
                                        </List>
                                    </Tab>
