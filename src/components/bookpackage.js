...
class BookPackage extends Component {
    ...
    async getPackage(id) {
        try {
            ...
            let url = 'http://upkon.ir/api/ApiPackages/DetailsPackages';
            ...
            fetch(url, {
                ...
                .then((responseJson) => {
                    ...
                    if (responseJson.Ok === true) {
                        this.setState({
                            package: responseJson.PackageDetails
                        });
                    } else {
                        this.setState({loading: false});
                        Toast.show({
    ...
    componentDidMount() {
        let PackageId = this.props.navigation.state.params.PackageId;
    ...
    async book() {
        ...
        if (allow) {
            ...
            let token = await AsyncStorage.getItem('token');
            fetch('http://upkon.ir/api/ApiPackages/Rezerv', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                ...
                <Drawer ref={(ref) => {
                    ...
                    <Container>
                        <Header style={{backgroundColor: '#ffffff'}}>
                            ...
                            <Right>
                                <Title style={styles2.headerTitleText}>رزرو پکیج</Title>
                            ...
                            <Card>
                                <CardItem header>
                                    ...
                                    <Text style={{
                                        fontFamily: 'IRANSans(FaNum)',
                                        fontSize: 12
                                    }}>{this.state.package == null ? "نامشخص " : this.state.package.CategoryName + " "}</Text>
                                    <Icon type="FontAwesome" name="chevron-left"
                                          style={{color: '#bed73c', fontSize: 16, marginTop: 7}}/>
                                    <Text style={{
                                        fontFamily: 'IRANSans(FaNum)',
                                        fontSize: 12
                                    }}>{this.state.package == null ? "نامشخص " : this.state.package.Name + " "}</Text>
                                    </Body>
                                </CardItem>
                                <CardItem cardBody>
                                    <Image style={{height: 200, width: null, flex: 1}}
                                           source={this.state.package !== null && this.state.package.ImageUrl !== null ? {uri: String('http://upkon.ir/' + this.state.package.ImageUrl)} : require('../img/1.png')}/>
                                </CardItem>
                                <CardItem>
                                    <Text style={{
                                        fontFamily: 'IRANSans(FaNum)',
                                        fontSize: 12
                                    }}>{this.state.package == null ? "" : this.state.package.Descrition + " "}</Text>
                                </CardItem>
                            </Card>
